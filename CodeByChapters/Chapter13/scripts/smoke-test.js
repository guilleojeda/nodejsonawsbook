...
const preloadArg = process.argv.find((arg) => arg.startsWith('--preload='));
const nodeArgs = [];

if (preloadArg) {
  nodeArgs.push('-r', preloadArg.slice('--preload='.length));
}

nodeArgs.push('app.js');
...
async function main() {
  const child = spawn(process.execPath, nodeArgs, {
    ...
    env: {
      ...process.env,
      PORT: port,
      OTEL_SERVICE_NAME: process.env.OTEL_SERVICE_NAME || 'SimpleAWSNodejsApp',
